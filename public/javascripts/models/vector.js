...
class Vector extends Array {
  ...
  // Rotate point counter-clockwise around the axis by angle θ
  rotate (axis, θ) {
    const angle = θ / 2
    const scalar = Math.cos(angle)
    const sinθ = Math.sin(angle)
    const vector = axis.multiply(sinθ)
    const polarQuaternion = new Quaternion(Vector.from([scalar, ...vector]))
    const polarInverse = polarQuaternion.invert()
    const quaternion = numberSystems.toQuaternion(this)
    const result = polarQuaternion.multiply(quaternion).multiply(polarInverse)

    return numberSystems.toPoint(result)
  }
